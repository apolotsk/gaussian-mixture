...
def target_θ():
  ...
  variances = np.expand_dims([2.0, 1.0], axis=0)
...
def x(target_θ):
  ...
  samples = np.random.normal(means, variances**0.5, [x_length, 1, z_length])
...
def θ():
  ...
  variances = np.random.rand(1, z_length)*10+1
  return P_z, means, variances
θ = θ()
...
while abs(log_likelihood1-log_likelihood0)>1e-6:
  ...
  def maximization_step(P_z_given_x, x):
    ...
    variances = (P_z_given_x*(x-means)**2).sum(axis=0, keepdims=True)/w
    return P_z, means, variances
  ...
  def show(x):
    ...
    P_z, means, variances = θ
    y = P_x(means.T, θ).T
    pyplot.plot(means + [[-1],[1]]*variances**0.5, y/[[2],[2]], '|-b', alpha=0.3, linewidth=1)
