...
def target_θ():
  ...
  variances = np.expand_dims([2.0, 1.0], axis=1)
  return P_z, means, variances
target_θ = target_θ()

def x(target_θ):
  P_z, means, variances = target_θ
  z = np.random.choice(z_length, size=x_length, p=P_z[:,0])
  samples = np.random.normal(means, variances**0.5, [z_length, x_length])
  return np.choose(z, samples)
x = x(target_θ)

def θ():
  def normalize(x, axis=0): return x/x.sum(axis=axis, keepdims=True)
  P_z = normalize(np.random.rand(z_length, 1)*0.5+0.25)
  means = np.random.rand(z_length, 1)*20-10
  variances = np.random.rand(z_length, 1)*10+1
  return P_z, means, variances
θ = θ()
...
while abs(log_likelihood1-log_likelihood0)>1e-6:
  def expectation_step(x, θ):
    P_z, means, variances = θ
    P_x_given_z = gauss(x, means, variances)
    P_x_and_z = P_z*P_x_given_z
    P_x = P_x_and_z.sum(axis=0, keepdims=True)
  ...
  def maximization_step(P_z_given_x, x):
    ...
    variances = (P_z_given_x*(x-means)**2).sum(axis=1, keepdims=True)/w
    return P_z, means, variances
  ...
  def show(x):
    ...
    P_z, means, variances = θ
    y = P_x(means.T, θ)[None].T
    pyplot.plot((means + [-1,1]*variances**0.5).T, (y/[2,2]).T, '|-b', alpha=0.3, linewidth=1)
    pyplot.plot(([1,1]*means).T, ([0,1]*y).T, '|-b', alpha=0.3, linewidth=1)
